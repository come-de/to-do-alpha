            ...
            return <article className={`coverage-card ${view}`} key={tutor.tutorId}>
              ...
              <div className="coverage-slots available"><strong>Créneau(x) mobilisable(s) sans chevauchement</strong>{tutor.opportunities.slice(0, 8).map((row, index) => {
                ...
                return <div className="coverage-opportunity" key={`${row.sessionId}-${row.timeSlot}-${index}`}>
                  <span className="coverage-opportunity-school" title={`${row.timeSlot || "Horaire non précisé"} · ${row.school || "Établissement non précisé"} · ${schoolOwnerLabels[ownerForSchool(row.school)]}`}>{row.timeSlot || "Horaire non précisé"} · <SchoolAdminLink schoolId={schoolRecordForName(row.school)?.externalId}>{row.school || "Établissement non précisé"}</SchoolAdminLink> · {schoolOwnerLabels[ownerForSchool(row.school)]}</span>
